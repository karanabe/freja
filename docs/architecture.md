...
optional `ServerApp` transport adapter; the production CLI currently uses the
pure-Tokio adapter behind the same engine boundary.
...
an explicit fail-open or fail-closed policy, while the separate bounded UI
publisher may drop immutable snapshots and increments a drop metric. Payload
capture, hooks, remote exposure, and TLS interception are opt-ins.
